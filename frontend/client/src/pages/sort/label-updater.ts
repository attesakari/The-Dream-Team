...
export const updateStudentsLabels = (
  projectName: string,
  event: DragEndEvent,
): void => {
  ...
  const newColumn = columnIdToLabelType(
    (JSON.parse(over.id as string) as DragID).columnId,
  );
  ...
  if (oldColumn === LabelType.Selected) {
    removeLabelFromStudent(card.cardId!, {
